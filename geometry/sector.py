...
class Sector:
    ...
    def __init__(self, radius: float, center: Point, rotation_angle: float, arc_angle: float):
        ...
        # сам угол сектора (дуга, на которую он опирается).
    ...
    def is_inside(self, p: Point) -> bool:
        ...
        mult1 = cross_product(get_vector(self.__center, p),
                              get_vector(self.__center, self.__p1))
        if mult1 > 0:
            return False

        mult2 = cross_product(get_vector(self.__center, p),
                              get_vector(self.__center, self.__p2))
        return mult2 >= 0
    ...
    def process_draw(self, screen):
        ...
        Отрисовка для debug
        """
        draw.line(screen, (0, 0, 255), [self.__center.x, self.__center.y], [
                  self.__p1.x, self.__p1.y], 10)
        draw.line(screen, (0, 0, 255), [self.__center.x, self.__center.y], [
                  self.__p2.x, self.__p2.y], 10)
