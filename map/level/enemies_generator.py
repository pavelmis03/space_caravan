...
def create_enemy(grid, i: int, j: int, weapon_name: str):
    ...
    """
    enemy = Enemy(grid.scene, grid.controller, weapon_name,
...
class EnemyGenerator:
    ...
    def generate(self):
        ...
        for item in self.__rectangles:
            ...
            random_i = randint(item.top_index + WALLS_MIN_DISTANCE,
                               item.bottom_index - WALLS_MIN_DISTANCE)
            random_j = randint(item.left_index + WALLS_MIN_DISTANCE,
                               item.right_index - WALLS_MIN_DISTANCE)
