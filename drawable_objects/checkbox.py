...
class CheckBox(DrawableObject):
    ...
    def move_to(self, position):
        ...
        if self.align == 'center':
            half_w = (self.select.width +
                      self.label.text_surface.get_width()) / 2
            movement.x -= half_w
        self.move(movement)
    ...
    def process_draw(self):
        pygame.draw.rect(self.scene.screen, CheckBox.BG_COLOR,
                         rectangle_to_rect(self.select))
        if self.check:
            pygame.draw.rect(
                self.scene.screen, CheckBox.BG_ENABLED_COLOR, rectangle_to_rect(self.selected))
